...
(function () {
    ...
    _input.methods.qrcode = {
        ...
        makeCode: function (field, value, text, placeholder) {
            var qrCodeDef = field.tagged(/input-qrcode-(\d+)x(\d+)?/),
                width = qrCodeDef ? parseInt(qrCodeDef[1]) : 128,
                height = qrCodeDef ? parseInt(qrCodeDef[2]) : 128,
            ...
            placeholder.css({ width: width, height: height, display: 'inline-block', padding: 1, 'background-color': colorLight });
            if (value != null) {
                try {
                    ...
                    placeholder.css('visibility', '').find('img').css({
